...
make_individual_plots <- function(filename, df, pars_to_plot, xaxis_labs, experiment, facet_var, png_width) {
  ...
  # png(paste0('Figures/hab_closure/', filename, '.png'), res = 500, units = 'in', width = 7,
  #     height = 2*length(pars_to_plot))
  ggsave(filename = paste0('Figures/hab_closure/', filename, '.png'), 
...
png('Figures/hab_closure/access_individuals_agg.png', width = 4, height = 4, res = 500, units = 'in')
access_tibbles$income.summary %>%
...
write.csv(to.print, file = 'Figures/hab_closure/results_summary.csv')
...
write.csv(to.print, file = 'Figures/hab_closure/results_summary_supp.csv')

# effect of synchrony on portfolio effects --------------------------------

png('Figures/hab_closure/portfolio_benefits.png', width = 7, height = 4, units = 'in', res = 500)
sync_tibbles$income.summary %>%
  prep_individuals_to_plot(experiment = 'synchrony') %>%
...
to.save <- map_dfr(fifty.sims, function(scenario) map_dfr(scenario, ~.x$groundfish_bio[,1]), 
  ...
  NULL
ggsave('Figures/hab_closure/groundfish_check.png', height = 8, width = 6, units = 'in', dpi = 500)
...
ggsave('Figures/hab_closure/catch_dynamics.png', height = 6, width = 7, units = 'in', dpi = 500)
